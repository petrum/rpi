...
def alarm(p):
    ...
    if ago > limit:
        ...
        if len(sys.argv) > 1:
            address = sys.argv[1]
            cmd = "echo -e 'Subject: motion detected on {} {}\r\n\r\nPIR motion detection at {}' | /usr/bin/msmtp {}".format(socket.gethostname(), address, st, address)
            os.system(cmd)
            print(st, "Sent email to", address, file=sys.stderr)
    else:
        print(st, "Skipped (", ago, 'sec ago <', limit, 'sec )', file=sys.stderr)
